...
export default function NewPasswordOTP({route, navigation}) {
  const toasts = useRef();
  ...
  const handleVerifyOTP = async () => {
    if (otp_code == '') {
      ...
      try {
        setLoading(true);
        const response = await fetch(Api_path + 'verify-reset-password-otp', {
          method: 'POST',
          headers: {
        ...
        console.log(data, 'verify-reset-password-otp Api Response');
